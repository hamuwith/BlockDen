...
public class BreakToolDataSOEditor : Editor
{
    ...
    void DrawRecipeGrid(BreakToolDataSO so, BreakToolData item)
    {
        EnsureSlots(so, item);
        ...
        for (int row = GridRows - 1; row >= 0; row--)
        {
            ...
            for (int col = 0; col < GridCols; col++)
            {
                ...
                var newCat = (Item.ItemCategory)EditorGUILayout.EnumPopup(slot.Category, GUILayout.Width(CellWidth - 4));
    ...
    void EnsureSlots(BreakToolDataSO so, BreakToolData item)
    {
        ...
        for (int i = copy; i < needed; i++)
            newSlots[i] = new ItemAccess { Category = Item.ItemCategory.Material, Id = -1, Num = 0 };
    ...
    void SaveCSV(BreakToolDataSO so)
    {
        ...
        sb.Append("Name,Category,Id,UnitNum,MaxNum,BlockType,Lv,BreakPower");
        ...
        foreach (var d in so.ItemDatas)
        {
            ...
            sb.Append($",{(int)d.BlockType},{d.Lv},{d.BreakPower}");
        ...
        Debug.Log($"BreakToolDataをCSVに保存しました: {path}");
    }
    ...
    void LoadCSV(BreakToolDataSO so)
    {
        ...
        for (int li = 1; li < lines.Length; li++)
        {
            ...
            ItemAccess[] recipeSlots = ParseRecipeSlots(cols, headerMap);

            list.Add(new BreakToolData
            {
                Name = name,
    ...
    ItemAccess[] ParseRecipeSlots(string[] cols, Dictionary<string, int> map)
    {
        ...
        for (int m = 0; m < slots.Length; m++)
        {
            ...
            if (matId < 0)
            {
                slots[m] = new ItemAccess { Category = Item.ItemCategory.Material, Id = -1, Num = 0 };
                continue;
            }
            slots[m] = new ItemAccess
            {
                Category = (Item.ItemCategory)ReadInt(cols, map, $"Mat{m}_Category", (int)Item.ItemCategory.Material),
                Id       = matId,
