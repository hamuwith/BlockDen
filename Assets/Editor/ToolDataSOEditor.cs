...
[CustomEditor(typeof(ToolDataSO))]
public class ToolDataSOEditor : Editor
{
    public override void OnInspectorGUI()
    {
        DrawDefaultInspector();
        if (GUILayout.Button("Load from CSV"))
            LoadCSV((ToolDataSO)target);
    }

    void LoadCSV(ToolDataSO so)
    {
        ...
        for (int li = 1; li < lines.Length; li++)
        {
            ...
            list.Add(new ToolData
            {
                ...
                CraftCategory = (ItemCategory)int.Parse(Col(cols, map, "CraftCategory")),
                ItemMaterials = ParseMaterials(cols, map)
            });
        }
        so.SetItemDatas(list.ToArray());
        AssetDatabase.SaveAssets();
        Debug.Log($"{list.Count}件のToolDataを読み込みました");
    }
